...
if(!abc2svg)
var abc2svg={}
function ToAudio(){return{add:function(first,voice_tb,cfmt){var toaud=this,C=abc2svg.C,p_time=0,abc_time=0,play_fac=C.BLEN/4*120/60,i,n,dt,d,v,s=first,rst=s,rst_fac,rsk=[],b_tim,b_typ,instr=[],chn=[]
...
rst.bar_type+=':'
set_variant(s)
rst_fac=play_fac}}else if(s.rbstop==2){if(s.bar_type.slice(-1)==':'){if(b_typ&4)
break
...
if(s2){po.repn=false}else{s2=var_end(s)
if(s2==po.s_end)
break}}
...
if(--w_instr==0)
play_start()})}
function def_instr(s,f){var i
for(;s;s=s.ts_next){if(s.subtype=="midiprog"){po.v_c[s.v]=s.chn
i=s.instr
if(i==undefined)
continue
po.c_i[s.chn]=i}else if(!s.dur){continue}else{i=po.v_c[s.v]
if(i==undefined)
...
r.send()
return}
def_instr(s,sf2_create)}else{w_instr++
def_instr(s,load_instr)}}
function get_time(po){return po.ac.currentTime}
function midi_ctrl(po,s,t){if(s.ctrl==7)
s.p_v.vol=s.val/127}
function midi_prog(po,s){po.v_c[s.v]=s.chn
if(s.instr)
po.c_i[s.chn]=s.instr}
function note_run(po,s,key,t,d){var g,st,c=po.v_c[s.v],instr=po.c_i[c],k=key|0,parm=po.params[instr][k],o=po.ac.createBufferSource(),v=s.p_v.vol==undefined?1:s.p_v.vol
if(!v||!parm)
return
...
po={conf:conf,onend:conf.onend||empty,onnote:conf.onnote||empty,s_end:i_end,s_cur:i_start,repv:i_lvl||0,tgen:2,get_time:get_time,midi_ctrl:midi_ctrl,midi_prog:midi_prog,note_run:note_run,timouts:[],v_c:[],c_i:[],ac:ac,gain:gain,params:params,rates:rates}
...
return
i=po.c_i[c]
if(i==undefined){po.op.send(new Uint8Array([0xb0+c,121,0]))
if(s.p_v.midictl){for(i in s.p_v.midictl)
